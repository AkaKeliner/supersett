...
const DrillToDashboardMenuItems = ({
  ...
  const goToDashboard = (filter: DDChart) => {
    ...
    const hidIndex = Number.parseInt(url.hid_index, 10) || 0;
    ...
    try {
      ...
      sessionStorage.setItem(
        preselect_filters_key,
        JSON.stringify(preselect_filters),
      );
      const dataKey = shortid.generate();
      const data = { extra_where: newFormData.where, filters: newFilters };
      sessionStorage.setItem(dataKey, JSON.stringify(data));
      window.location.href = `/superset/dashboard/${
        filter.url
      }/?preselect_filters_key=${preselect_filters_key}&hid=${hid}&hid_index=${
        hidIndex + 1
      }&data_key=${dataKey}`;
    } catch (err) {
      //   console.log(err);
